...
public final class AnvilRecipe {
        ...
        CraftTweakerAPI.apply(new IRuntimeAction() {
            ...
            public void apply() {
                AARecipeHelper.getRecipes(AARecipeTypes.ANVIL)
                        ...
                        .filter(r -> r.getResultItem(Minecraft.getInstance().getConnection().registryAccess()).is(result.getInternal().getItem()))
                        .filter(r -> r.getIngredients().get(0) == leftInput.asVanillaIngredient() && r.getIngredients().get(1) == rightInput.asVanillaIngredient())
                        .filter(r -> r.getCounts().get(0) == leftInputCount && r.getCounts().get(1) == rightInputCount)
                        .map(Recipe::getId)
                        .forEach(r -> AARecipeHelper.getRecipes(AARecipeTypes.ANVIL).remove(r));
            }
        ...
        CraftTweakerAPI.apply(new IRuntimeAction() {
            ...
            public void apply() {
                AARecipeHelper.getRecipes(AARecipeTypes.ANVIL)
                        ...
                        .filter(r -> r.getInputNbt(0).equals(leftInput.getInternal().getOrCreateTag()) && r.getInputNbt(1).equals(rightInput.getInternal().getOrCreateTag()))
                        .map(Recipe::getId)
                        .forEach(r -> AARecipeHelper.getRecipes(AARecipeTypes.ANVIL).remove(r));
            }
    ...
    public static void removeRecipe(String id) {
        CraftTweakerAPI.apply(new IRuntimeAction() {
            @Override
            public void apply() {
                AARecipeHelper.getRecipes(AARecipeTypes.ANVIL).remove(new ResourceLocation(AnvilAPI.MOD_ID, id));
            }
    ...
    public static void removeRecipe(String modId, String id) {
        CraftTweakerAPI.apply(new IRuntimeAction() {
            @Override
            public void apply() {
                AARecipeHelper.getRecipes(AARecipeTypes.ANVIL).remove(new ResourceLocation(modId, id));
            }
