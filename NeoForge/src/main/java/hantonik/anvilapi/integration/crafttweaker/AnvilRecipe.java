        ...
        CraftTweakerAPI.apply(new IRuntimeAction() {
            ...
            public void apply() {
                AARecipeHelper.getRecipes()
                        ...
                        .map(RecipeHolder::id)
                        .forEach(r -> AARecipeHelper.getRecipes().get(AARecipeTypes.ANVIL.get()).remove(r));
            }
        ...
        CraftTweakerAPI.apply(new IRuntimeAction() {
            ...
            public void apply() {
                AARecipeHelper.getRecipes()
                        ...
                        .filter(r -> r.value().getResultItem(ServerLifecycleHooks.getCurrentServer().registryAccess()).is(result.getInternal().getItem()))
                        .filter(r -> r.value().getIngredients().get(0) == leftInput.asVanillaIngredient() && r.value().getIngredients().get(1) == rightInput.asVanillaIngredient())
                        .filter(r -> ((IAnvilRecipe) r.value()).getCounts().get(0) == leftInputCount && ((IAnvilRecipe) r.value()).getCounts().get(1) == rightInputCount)
                        .map(RecipeHolder::id)
                        .forEach(r -> AARecipeHelper.getRecipes().get(AARecipeTypes.ANVIL.get()).remove(r));
            }
        ...
        CraftTweakerAPI.apply(new IRuntimeAction() {
            ...
            public void apply() {
                AARecipeHelper.getRecipes()
                        ...
                        .filter(r -> ((IAnvilRecipe) r.value()).getInputNbt(0).equals(leftInput.getInternal().getOrCreateTag()) && ((IAnvilRecipe) r.value()).getInputNbt(1).equals(rightInput.getInternal().getOrCreateTag()))
                        .map(RecipeHolder::id)
                        .forEach(r -> AARecipeHelper.getRecipes().get(AARecipeTypes.ANVIL.get()).remove(r));
            }
