            ...
            for (var enchantmentJson : json.get("enchantments").getAsJsonArray()) {
                ...
                if (enchantmentJson.isJsonObject()) {
                    if (enchantmentJson.getAsJsonObject().has("combining")) {
                        ...
                        enchantment1 = ForgeRegistries.ENCHANTMENTS.getValue(new ResourceLocation(GsonHelper.getAsString(combiningJson.get(0).getAsJsonObject(), "enchantment")));
                        enchantment1Level = GsonHelper.getAsInt(combiningJson.get(0).getAsJsonObject(), "level", -1);

                        if (combiningJson.size() >= 2) {
                            enchantment2 = ForgeRegistries.ENCHANTMENTS.getValue(new ResourceLocation(GsonHelper.getAsString(combiningJson.get(1).getAsJsonObject(), "enchantment")));
                    ...
                    enchantment1 = ForgeRegistries.ENCHANTMENTS.getValue(new ResourceLocation(enchantmentJson.getAsString()));

                if (enchantment2 == null) {
                    if (baseItem == Ingredient.EMPTY)
                        AnvilAPI.LOGGER.debug("Disabling enchantment recipe with {}", ForgeRegistries.ENCHANTMENTS.getKey(enchantment1));
                    else
                        for (var baseStack : baseItem.getItems())
                            AnvilAPI.LOGGER.debug("Disabling enchantment recipe for {} with {}", ForgeRegistries.ITEMS.getKey(baseStack.getItem()), ForgeRegistries.ENCHANTMENTS.getKey(enchantment1));

                    ENCHANTMENTS.add(Pair.of(Pair.of(enchantment1, enchantment1Level), baseItem));
                } else {
                    AnvilAPI.LOGGER.debug("Disabling enchantment combining recipe for {} with {}", ForgeRegistries.ENCHANTMENTS.getKey(enchantment1), ForgeRegistries.ENCHANTMENTS.getKey(enchantment2));
